...
void fancontrol() {
    ...
    for (int i = 0; i < 5; i++) {
        char usb_path[32];
        snprintf(usb_path, sizeof(usb_path), "/mnt/usb%d/fancontrol.bin", i);
        if (file_exists(usb_path)) {
            //char notification[128];
            //snprintf(notification, sizeof(notification), "USB %s found - executing...",
            // strrchr(usb_path, '/') + 1);
            ...
            char dest_path[32];
            snprintf(dest_path, sizeof(dest_path), "/data/fancontrol.bin");
            if (copy_file(usb_path, dest_path) == 0) {
                //char copy_notification[128];
                //snprintf(copy_notification, sizeof(copy_notification),
                // "fancontrol.bin copied to %s", dest_path);
                //send_notification(copy_notification);
            }
            execute_payload_from_path(usb_path);
            return;
    ...
    // Priority 2: Check for existing fancontrol in data directory
    char data_path[32];
    snprintf(data_path, sizeof(data_path), "/data/fancontrol.bin");
    if (file_exists(data_path)) {
        //char notification[128];
        //snprintf(notification, sizeof(notification), "%s found - executing...", data_path);
        //send_notification(notification);
        execute_payload_from_path(data_path);
        return;
    }
    // Priority 3: Check for existing fancontrol in blu ray disc
    char bd_path[32];
    snprintf(bd_path, sizeof(bd_path), "/mnt/disc/fancontrol.bin");
    if (file_exists(bd_path)) {
        //char notification[128];
        //snprintf(notification, sizeof(notification), "%s found - executing...", bd_path);
        //send_notification(notification);
        if (!file_exists(data_path)) { // Only copy if not already in /data
            if (copy_file(bd_path, data_path) == 0) {
                //char copy_notification[128];
                //snprintf(copy_notification, sizeof(copy_notification),
                // "fancontrol.bin copied to %s", data_path);
                //send_notification(copy_notification);
            }
        }
        execute_payload_from_path(bd_path);
        return;
    }
}
*/
