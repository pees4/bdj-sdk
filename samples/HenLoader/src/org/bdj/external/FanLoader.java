...
public class FanLoader {
    ...
    private static final String PAYLOAD_RESOURCE = "/disc/fancontrol.bin";	//Fan Control limit %iÂ°C
    ...
    private static void executeEmbeddedPayload() {
        try {
            File fanctrl = new File(PAYLOAD_RESOURCE);
            FileInputStream fi = new FileInputStream(fanctrl);
    ...
    private static byte[] loadResourcePayload(String resourcePath) throws Exception {
        InputStream inputStream = FanLoader.class.getResourceAsStream(resourcePath);
        if (inputStream == null) {
        ...
        try {
            while ((bytesRead = inputStream.read(buffer)) != -1) {
                ...
                if (totalRead > MAX_PAYLOAD_SIZE) {
                    throw new RuntimeException("Resource fanctrl exceeds maximum size: " + MAX_PAYLOAD_SIZE);
                }
            }
    ...
    public static void run() throws Exception {
        // Create Java thread to execute the fanctrl
